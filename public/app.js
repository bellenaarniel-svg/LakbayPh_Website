...
function wireForms() {
  ...
  $('yearSelect').addEventListener('change', () => loadSummary($('yearSelect').value));
...
async function openDashboard() {
  ...
  await loadBookings();
  await loadSummary(new Date().getFullYear());
  await loadPricing();
}
...
function renderPriceRoutes(routes) {
  ...
  $('priceRoutesBody').innerHTML = routes.map(r => `
    <tr>
      ...
      <td><button class="save-price" onclick="saveRoutePrice(${r.id})" ${dis}>Save</button></td>
    </tr>
  `).join('');
}
...
function renderPriceVans(vans) {
  ...
  $('priceVansBody').innerHTML = vans.map(v => `
    <tr>
      ...
      <td><button class="save-price" onclick="saveVanMultiplier(${v.id})" ${dis}>Save</button></td>
    </tr>
  `).join('');
}
...
async function saveRoutePrice(id) {
...
async function saveVanMultiplier(id) {
...
async function updateStatus(id, status) {
  ...
  await loadBookings();
  await loadSummary($('yearSelect').value || new Date().getFullYear());
}
...
async function loadSummary(year) {
  ...
  $('yearSelect').innerHTML = years.map(y => `<option value="${y}" ${y == data.year ? 'selected' : ''}>${y}</option>`).join('');
  $('chartYear').textContent = data.year;
